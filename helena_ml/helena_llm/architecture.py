...
── Memory efficiency changes (v2) ───────────────────────────────────────────
The original architecture materialized dA and dB as full [B,L,d_inner,d_state]
tensors before the SSM scan loop. For BASE config this created tensors of ~134MB
per layer in fp16, and PyTorch stored all 12 layers' tensors for backpropagation
— consuming ~3.2GB before any activations were counted.

Fix 1 — Per-step dA/dB: compute discretized A and B one timestep at a time
inside the scan loop rather than pre-computing the full sequence tensor. Peak
memory drops from O(L * d_inner * d_state) to O(d_inner * d_state) per layer.

Fix 2 — Gradient checkpointing: optional recomputation of block activations
during backward pass instead of storing them. Halves activation memory at the
cost of ~33% longer backward pass. Enable via model.enable_gradient_checkpointing().

These two fixes together allow BASE (334M params) to train on a single T4 (15GB)
with batch_size=4 and mixed precision. Without them, BASE OOMs on the first step.
─────────────────────────────────────────────────────────────────────────────
"""
import math
import torch
...
class LIFNeuron(nn.Module):
    ...
    def forward(self, current: torch.Tensor,
        ...
        Returns:
            ...
            membrane: updated membrane potential [batch, d_model]
        """
...
class SSMLayer(nn.Module):
    ...
    Memory note: dA and dB are computed per-step (not as full [B,L,d_inner,d_state]
    tensors) to avoid materializing ~134MB tensors per layer in fp16.
    """
    ...
    def __init__(self, config: HelenaNetConfig):
        ...
        # Store dimensions as attributes — cleaner than referencing shapes
        ...
        # Input-dependent B, C (selectivity — the key Mamba innovation)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # SSM parameters
        A_neg = -torch.exp(self.A_log)                 # [d_inner, d_state] — negative decay

        # Input-dependent parameters (computed for all positions at once — these
        # are [B, L, *] not [B, L, d_inner, d_state] so they're fine to precompute)
        ...
        # ── SSM scan ─────────────────────────────────────────────────────────
        # Compute dA and dB PER STEP, not as full [B,L,d_inner,d_state] tensors.
        #
        # Original code precomputed:
        #   dA = torch.exp(dt.unsqueeze(-1) * A.unsqueeze(0).unsqueeze(0))
        #   → [B, L, d_inner, d_state] = ~134MB fp16 for BASE, stored by autograd
        #     across all 12 layers = ~3.2GB. This is the cause of OOM.
        #
        # Fix: compute dA_t = exp(dt_t ⊗ A) and dB_t = dt_t ⊗ B_t per step.
        # Per-step tensors are [B, d_inner, d_state] = ~0.5MB. Never accumulate.
        ...
        for t in range(seq):
            ...
            # Discretize A and B for this timestep (zero-order hold)
            dA_t = torch.exp(dt_t.unsqueeze(-1) * A_neg.unsqueeze(0))  # [B, d_inner, d_state]
            dB_t = dt_t.unsqueeze(-1) * B_t.unsqueeze(1)               # [B, d_inner, d_state]

            # State update: h(t) = dA * h(t-1) + dB * x(t)
...
class SparseMoELayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return output.view(batch, seq, d) + residual
...
class SpikingSSMMoEBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        Args:
            x:        [batch, seq, d_model]
            membrane: LIF membrane state from previous call (None = fresh start)
        Returns:
            x:        transformed [batch, seq, d_model]
            membrane: updated membrane state [batch, d_model]
        """
...
class HelenaNet(nn.Module):
    ...
    Replaces Mistral in HybridLLM when trained.
    """
    ...
    def __init__(self, config: HelenaNetConfig):
        ...
        # Gradient checkpointing flag — set via enable_gradient_checkpointing()
        # When enabled, block activations are recomputed during backward instead
        # of stored, halving activation memory at the cost of ~33% slower backward.
    ...
    def enable_gradient_checkpointing(self):
        ...
        Reduces activation memory by roughly half at the cost of ~33% slower
        backward pass. Recommended for BASE and LARGE configs on limited VRAM.
        """
    ...
    def forward(self, input_ids: torch.Tensor,
        ...
        Args:
            input_ids: [batch, seq]
            targets:   [batch, seq] for training (shifted input_ids), else None
        ...
        for i, block in enumerate(self.blocks):
            if self.gradient_checkpointing and self.training:
                ...
                # instead of storing them. Saves ~50% activation memory.
                # Note: membrane is always None during training (fresh each step).
                def make_block_fn(blk):
                    def block_fn(x_in):
                        return blk(x_in, None)
                    return block_fn

                x, membrane = checkpoint(
                    make_block_fn(block), x,
                    use_reentrant=False,
                )
            else:
